...
# ImageNet normalization (for ResNet-style backbones)
...
class VideoClipDataset(Dataset):
    """
    Samples fixed-length clips from videos listed in a manifest.

    CSV manifest header example (clip-level):
    ...
    JSON manifest example (frame-level, Option B):
        [
          {
            "video": "datasets/pendulum/clip1.mp4",
            "image_path": "...",
            "frame_idx": 0,
            "scenario": "hanging",
            ...
    ...
    In that case, we collapse to one row per unique "video" and treat that as "path".
    ...
    Each sample: dict with
        ...
        "video_path": original video path (str)
    """
    ...
    def __init__(
        ...
        # Option B: if manifest is JSON frame-level (has "video" but no "path"),
        # collapse to one row per unique video.
        first_row = self.rows[0]
        if "path" not in first_row and "video" in first_row:
                ...
                if v not in by_video:
                    ...
                    for k in [
                        ...
                        if k in r:
                            out[k] = r[k]
                    by_video[v] = out
            self.rows = list(by_video.values())
        ...
        for i, row in enumerate(self.rows):
            ...
            # For sequential sampling, index all possible starts.
            # For random, index at least one start per video.
    ...
    def _load_clip(self, vpath: str, start: int) -> np.ndarray:
        ...
        cap.release()
        if len(frames) == 0:
            raise RuntimeError(f"No frames read from {vpath}")
    ...
    def __getitem__(self, idx: int) -> Dict[str, object]:
        ...
        if self.sampling == "random":
            ...
            if T_total >= self.clip_len:
                start = self.rng.randint(0, max(0, T_total - self.clip_len))
            else:
                start = 0
        ...
        # Normalize for ResNet-style backbone
