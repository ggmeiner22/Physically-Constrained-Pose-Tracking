...
def build_args() -> argparse.Namespace:
    ...
    p.add_argument("--outdir", type=str, default="outputs/position",
                   help="Where to save position-net checkpoints")
...
def main() -> None:
    ...
    full_ds = PositionDataset(
        ...
        pad=args.pad,
        use_pred_boxes=False,  # training with GT boxes
        detector=None,
        transform=transform,
    )
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for crops, bbox_norm, pos in tqdm(train_loader, desc=f"Train {epoch}", leave=False):
            crops = crops.to(device)        # (B,3,Hc,Wc)
            bbox_norm = bbox_norm.to(device)  # (B,4)
            pos = pos.to(device)            # (B,3)

            preds = model(crops, bbox_norm)  # (B,3)
            loss = criterion(preds, pos)
        ...
        n_batches = 0
        with torch.no_grad():
            for crops, bbox_norm, pos in tqdm(val_loader, desc=f"Val {epoch}", leave=False):
                crops = crops.to(device)
                bbox_norm = bbox_norm.to(device)
                pos = pos.to(device)
                preds = model(crops, bbox_norm)
                loss = criterion(preds, pos)
        ...
        if val_loss < best_val:
            best_val = val_loss
            ckpt_path = os.path.join(args.outdir, "best_position.pt")
            torch.save(
